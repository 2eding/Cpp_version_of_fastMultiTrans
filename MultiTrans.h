...
//void cal_standard_deviation(Eigen::MatrixXd kinship, Eigen::MatrixXd snp, double Vg, double Ve, std::ofstream& std_devi);
...
void thread_func_std2(std::vector<double>& ret, std::vector<double>& mat_col_sum_divide_mat_rows,
    int mat_cols, int mat_rows, int begin, int end, Eigen::MatrixXd& mat, int th_num);
...
struct lmm_fit fitLMM(const VectorXd& Kva, const VectorXd& y, const MatrixXd& X,
    const bool reml, const bool check_boundary, const double tol);
